        ...
        for i in range(self.V):
            for j in range(i, self.V):
                if self.G[i][j]:
                    ...
                    linePoint = [node_X[i] - min(node_X) +1, node_Y[i] - min(node_Y) + 1, next_node_X - min(node_X) + 1, next_node_Y - min(node_Y) + 1]
                    # canvas1.create_line(node_X[i] - min(node_X) +1, node_Y[i] - min(node_Y) + 1, next_node_X - min(node_X) + 1, next_node_Y - min(node_Y) + 1)
                    # canvas1.create_line(linePoint[0], linePoint[1], linePoint[2], linePoint[3], fill = 'green', width = 5)
                    is_draw = True
                    for k in range(len(self.answer_node[0])):
                        if i == self.answer_node[0][k] and j == self.answer_node[1][k]:
                            canvas1.create_line(linePoint[0], linePoint[1], linePoint[2], linePoint[3], fill = 'red', width = 3)
                            is_draw = False

                    if is_draw:
                        canvas1.create_line(linePoint[0], linePoint[1], linePoint[2], linePoint[3])
